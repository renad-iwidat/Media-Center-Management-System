...
| **2. إعادة الصياغة** | `is_rewritten = true` (تم تنظيفه بالـ AI) | ✅ نعم |
| **3. في قسم التحرير** | `editorial_queue.status = 'pending'` | ✅ نعم |
| **4. موافقة المحرر** | المحرر يضغط "موافقة" | ✅ نعم |
| **5. عدم الرفض** | `status ≠ 'rejected'` | ✅ نعم |
...
| **1. اكتمال المحتوى** | ≥ 100 حرف + صورة | ✅ نعم |
| **2. إعادة الصياغة** | `is_rewritten = true` (تم تنظيفه بالـ AI) | ✅ نعم |
| **3. التصنيف آلي** | `categories.flow = 'automated'` | ✅ نعم |
| **4. موافقة تلقائية** | النظام يوافق تلقائياً (كل 10 دقائق) | ✅ نعم |
...
2. **يكون اتعمله rewriting** (`is_rewritten = true`)
3. **تتم الموافقة عليه**:
   - يدوياً: المحرر يوافق
   - آلياً: النظام يوافق (كل 10 دقائق)
4. **لم يُرفض** (`status ≠ 'rejected'`)
5. **لم يُنشر من قبل** (لا يوجد في `published_items`)

### ⚠️ الخبر لا يصل لقسم النشر إذا:

1. ❌ غير مكتمل (< 100 حرف أو بدون صورة)
2. ❌ لم تتم إعادة صياغته بعد (`is_rewritten = false`)
3. ❌ تم رفضه (`status = 'rejected'`)
4. ❌ لا يزال في انتظار المراجعة (`status = 'pending'`)
5. ❌ تم نشره مسبقاً
