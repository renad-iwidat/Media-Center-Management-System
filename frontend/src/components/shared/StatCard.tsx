export function StatCard({ label, value, icon: Icon, trend, variant = "default" }: {
  label: string; value: string | number; icon: any; trend?: string; variant?: string;
}) {
  return (
    <div className="bg-gradient-to-br from-white to-gray-50 rounded-3xl p-6 border border-gray-200 shadow-sm relative overflow-hidden group hover:border-gray-300 transition-all">
      <div className="absolute top-0 right-0 w-24 h-24 bg-blue-100/30 blur-3xl rounded-full -mr-12 -mt-12 group-hover:bg-blue-100/50 transition-all" />
      <div className="flex justify-between items-start relative z-10">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-gray-600 font-bold mb-2">{label}</p>
          <p className="text-3xl font-black text-gray-900">{value}</p>
          {trend && <span className="text-[10px] text-emerald-700 font-bold mt-1 block">{trend}</span>}
        </div>
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
          variant === "warning" ? "bg-amber-100 text-amber-600" :
          variant === "success" ? "bg-emerald-100 text-emerald-600" :
          "bg-blue-100 text-blue-600"
        }`}>
          <Icon size={24} />
        </div>
      </div>
    </div>
  );
}
