    ...
    <div className="space-y-6 pb-12 font-sans">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Select 
            className="w-48"
            options={[{ value: '', label: 'كل الأنواع' }, ...lookups.types.map(t => ({ value: t.id.toString(), label: t.name }))]}
            value={filters.type}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {content.map((c) => (
          <div 
            key={c.id} 
            className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden group hover:shadow-xl hover:shadow-slate-200/50 transition-all cursor-pointer flex flex-col"
