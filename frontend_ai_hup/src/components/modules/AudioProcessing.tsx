      ...
      <div className="flex gap-4 p-1 bg-white/5 rounded-2xl w-fit mr-auto ml-0 flex-row-reverse">
        <button
          ...
          className={`px-8 py-3 rounded-xl text-sm font-arabic transition-all flex items-center gap-2 ${
            activeMode === 'STT' ? 'bg-brand-accent text-white' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Mic size={18} />
          تحويل الصوت لنص (STT)
        ...
        <button
          ...
          className={`px-8 py-3 rounded-xl text-sm font-arabic transition-all flex items-center gap-2 ${
            activeMode === 'TTS' ? 'bg-brand-accent text-white' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Volume2 size={18} />
          تحويل النص لصوت (TTS)
                ...
                filteredMedia.map(item => (
                  <div 
                    ...
                    className={`p-3 rounded-xl border cursor-pointer transition-all flex items-center justify-between ${
                      selectedMediaId === item.id ? 'bg-brand-accent/10 border-brand-accent' : 'bg-white/[0.02] border-white/5 hover:border-white/10'
                    }`}
                  >
                    ...
                    {selectedMediaId === item.id && <Check size={16} className="text-brand-accent" />}
                  </div>
                ))
              ) : (
                  ...
                  {['Kore', 'Puck', 'Aoide'].map(v => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl text-xs border transition-all ${
                        voice === v ? 'bg-brand-accent border-brand-accent text-white' : 'bg-white/5 border-white/10 text-gray-500'
                      }`}
                    >
                      {v}
                    </button>
              ...
              audioUrl ? (
                <div className="space-y-8 w-full max-w-sm">
                  <div className="flex flex-col items-center gap-6">
                    <div className="w-24 h-24 bg-brand-accent/10 rounded-full flex items-center justify-center text-brand-accent relative">
                  ...
                  <div className="bg-white/5 rounded-3xl p-6 border border-white/10">
                    ...
                    <div className="flex items-center gap-4">
                      <button 
                        ...
                        className="w-14 h-14 bg-brand-accent rounded-full flex items-center justify-center text-white hover:scale-105 transition-transform"
                      >
                        {isPlaying ? <Pause size={24} /> : <Play size={24} className="translate-x-0.5" />}
                      </button>
                      <div className="flex-1 space-y-2">
                        <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                           <div className={`h-full bg-brand-accent ${isPlaying ? 'w-full' : 'w-0'}`} />
                        </div>
                      </div>
                    </div>
                  </div>
